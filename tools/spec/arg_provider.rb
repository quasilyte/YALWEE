module Yalwee::ArgProvider
  Arg = Yalwee::Arg

  class ValueParser
    def self.parse_immediate values
      values = parse values
  ...
  def int *values
    values = ValueParser.parse_immediate values
    return values.map {|x| (Arg.new 'int', x)}
  end

  def uint *values
    values = ValueParser.parse_immediate values
    return values.map {|x| (Arg.new 'uint', x)}    
  end
